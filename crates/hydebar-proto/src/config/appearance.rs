...
impl Default for MenuAppearance
{
    ...
    {
        Self {
            opacity: default_opacity(), backdrop: f32::default(),
        }
    }
}
...
pub struct Appearance
{
    ...
    #[serde(deserialize_with = "scale_factor_deserializer", default = "default_scale_factor")]
...
mod tests
{
    ...
    {
        let err_small: DeError =
            scale_factor_deserializer(F64Deserializer::<DeError,>::new(0.0,),)
                .expect_err("scale factor <= 0 should error",);
        assert!(err_small.to_string().contains("greater than 0.0"));

        let err_large: DeError =
            scale_factor_deserializer(F64Deserializer::<DeError,>::new(2.1,),)
