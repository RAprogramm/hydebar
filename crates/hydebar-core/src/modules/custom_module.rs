...
impl std::fmt::Display for CustomCommandError
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_,>,) -> std::fmt::Result
    {
        match self {
            ...
            Self::NonZeroExit {
                status,
            } => write!(f, "custom module process exited unsuccessfully ({:?})", status),
...
{
    sender.try_send(Message::Event(event,),).map_err(ModuleError::from,)
}
    ...
    M: 'static + Clone,
{
    ...
    ) -> Result<(), ModuleError,>
    {
        ...
        self.registration = config.and_then(|definition| {
            definition.listen_cmd.as_ref().map(|command| CustomRegistration {
    ...
    ) -> Option<(Element<'static, M,>, Option<OnModulePress<M,>,>,),>
    {
        ...
        let maybe_text_element = if let Some(error,) = &self.last_error {
            ...
            self.data.text.as_ref().and_then(|text_content| {
                if !text_content.is_empty() { Some(text(text_content.clone(),),) } else { None }
            },)
        };
        ...
        // NOTE: This returns None for action since we can't construct M in generic
        // code. The GUI layer should handle command launching based on module
        // configuration.
...
) -> Result<(), CustomListenerError,>
{
    let mut child = Command::new("bash",)
        ...
        .spawn()
        .map_err(
            |err| CustomListenerError::Command(CustomCommandError::Spawn(Arc::new(err,),),),
        )?;

    let stdout = child
        .stdout
        .take()
        .ok_or(CustomListenerError::Command(CustomCommandError::MissingStdout,),)?;
    ...
    match child.wait().await {
        Ok(status,) => {
            ...
            if status.success() {
                ...
                Err(CustomListenerError::Command(CustomCommandError::NonZeroExit {
                    status: status.code(),
                },),)
            }
        }
        Err(err,) => {
            Err(CustomListenerError::Command(CustomCommandError::Wait(Arc::new(err,),),),)
        }
    }
}
