...
async fn send_event_propagates_module_errors()
{
    ...
    let data = CustomListenData {
        alt: String::from("alt",), text: None,
    };

    sender.try_send(Message::Event(ServiceEvent::Update(data.clone(),),),).expect("initial send",);
...
"#,
    ...
    let first = receiver.try_recv().expect("first event",).expect("event present",);
    match first {
    ...
    let second = receiver.try_recv().expect("second event",).expect("event present",);
    match second {
        BusEvent::Module(ModuleEvent::Custom {
            name,
            message,
...
'; sleep 0.1; done"#,
    ...
    assert!(observed.iter().all(|(name, alt,)| { name.as_ref() == "second" && alt == "second" }));
}
