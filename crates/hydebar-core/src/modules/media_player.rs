...
impl MprisEventPublisher for MediaPlayerPublisher
{
    ...
    {
        Box::pin(ready(self.sender.try_send(Message::Event(event,),).map_err(ModuleError::from,),),)
    }
}
...
impl MediaPlayer
{
            ...
            Some(s,) => column!(
                ...
                column(s.iter().map(|d| {
                    ...
                    let volume_slider = d.volume.map(|v| {
                        slider(0.0..=100.0, v, move |v| Message::SetVolume(d.service.clone(), v,),)
                    },);
    ...
    fn handle_command(&mut self, service_name: String, command: PlayerCommand,)
    {
