        ...
        match message {
                            ...
                            match commands::check_for_updates(check_command.as_ref(),).await {
                                ...
                                Err(err,) => {
                                    ...
                                    if let Err(err,) = sender
                                        .try_send(Message::UpdatesCheckCompleted(Vec::new(),),)
                                    {
                                        error!(
            ...
            Message::Update(id,) => {
                if let (Some(runtime,), Some(sender,), Some(registration,),) =
                    (self.runtime.clone(), self.sender.clone(), self.registration.as_ref(),)
                {
...
mod tests
{
    ...
    fn register_spawns_hourly_task()
    {
        ...
        let config = UpdatesModuleConfig {
            check_cmd: ":".into(), update_cmd: ":".into(),
        };
    ...
    fn check_now_enqueues_result_on_event_bus()
    {
        ...
        while matches!(
            ...
            Some(BusEvent::Module(ModuleEvent::Updates(Message::UpdatesCheckCompleted(_))))
        ) {}
    ...
    fn toggle_updates_list_flips_visibility()
    {
        ...
        updates.update(Message::ToggleUpdatesList, &config, &mut outputs, &main_config,);
        assert!(updates.is_updates_list_open);
