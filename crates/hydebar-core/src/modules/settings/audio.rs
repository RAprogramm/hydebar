...
impl AudioData
{
    ...
    ) -> (Option<Element<'_, Message,>,>, Option<Element<'_, Message,>,>,)
    {
        ...
        if self.sources.iter().any(|source| source.in_use,) {
            let active_source =
                self.sources.iter().find(|source| source.name == self.server_info.default_source,);
    ...
    pub fn sinks_submenu(&self, id: Id, show_more: bool, opacity: f32,) -> Element<'_, Message,>
    {
        audio_submenu(
            self.sinks
                ...
                .collect(),
            if show_more { Some(Message::Audio(AudioMessage::SinksMore(id,),),) } else { None },
            opacity,
        )
    }
    ...
    -> Element<'_, Message,>
    {
        audio_submenu(
            self.sources
                ...
                .collect(),
            if show_more { Some(Message::Audio(AudioMessage::SourcesMore(id,),),) } else { None },
            opacity,
        )
    }
}
...
) -> Element<'a, Message,>
{
    Row::new()
        ...
        .push(slider(0..=100, volume, volume_changed,).step(1,).width(Length::Fill,),)
