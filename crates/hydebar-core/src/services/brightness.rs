...
impl BrightnessService
{
    ...
    async fn initialize_data(device_path: &Path,) -> Result<BrightnessData, BrightnessError,>
    {
        ...
        Ok(BrightnessData {
            current: actual_brightness, max: max_brightness,
        },)
    }
    ...
    async fn init_service() -> Result<(zbus::Connection, PathBuf,), BrightnessError,>
    {
        ...
        let conn = zbus::Connection::system().await.map_err(BrightnessError::from,)?;
    ...
    -> Result<AsyncFd<udev::MonitorSocket,>, BrightnessError,>
    {
        ...
        let builder = builder.match_subsystem("backlight",).map_err(BrightnessError::from,)?;
    ...
    fn backlight_enumerate() -> Result<Vec<udev::Device,>, BrightnessError,>
    {
        ...
        enumerator.match_subsystem("backlight",).map_err(BrightnessError::from,)?;

        Ok(enumerator.scan_devices().map_err(BrightnessError::from,)?.collect(),)
    }

    async fn start_listening<P,>(
        state: State,
        publisher: &mut P,
    ...
    {
        let brightness_ctrl =
            BrightnessCtrlProxy::new(conn,).await.map_err(BrightnessError::from,)?;
        let device_name = device_path.file_name().and_then(|d| d.to_str(),).ok_or_else(|| {
            BrightnessError::filesystem(format!("invalid device path: {}", device_path.display()),)
        },)?;
