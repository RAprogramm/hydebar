...
impl NotificationsService
{
    ...
    {
        self.storage.lock().unwrap().get_all().iter().cloned().collect()
    }
    ...
    fn subscribe() -> Subscription<ServiceEvent<Self,>,>
    {
        Subscription::run_with_id(
            ...
            stream::channel(100, |mut output| async move {
                ...
                // Send init event
                if output.send(ServiceEvent::Init(service.clone(),),).await.is_err() {
                ...
                if let Err(err,) =
                    connection.object_server().at("/org/freedesktop/Notifications", server,).await
                {
                ...
                if let Err(err,) = connection.request_name("org.freedesktop.Notifications",).await
                {
