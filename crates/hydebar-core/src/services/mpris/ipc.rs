...
use super::data::{MprisPlayerData, MprisPlayerMetadata, PlaybackStatus};
use super::dbus::MprisPlayerProxy;

/// Prefix applied to all MPRIS-compliant player service names on the session bus.
...
pub(crate) async fn build_event_stream(conn: &Connection) -> anyhow::Result<EventStream> {
    ...
    for entry in &data {
        ...
        combined.push(Box::pin(
            entry.proxy.receive_metadata_changed().await.filter_map({
