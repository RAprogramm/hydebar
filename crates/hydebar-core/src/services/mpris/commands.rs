...
use super::data::MprisPlayerData;
...
///
/// let command = MprisPlayerCommand::new("org.mpris.MediaPlayer2.Player".into(), PlayerCommand::Next);
...
pub(crate) async fn execute_player_command(
    ...
    let target = data
        .iter()
        .find(|entry| entry.service == command.service_name);
