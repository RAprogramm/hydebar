...
impl StatusNotifierItem
{
    pub async fn new(conn: &zbus::Connection, name: String,) -> anyhow::Result<Self,>
    {
        ...
        let icon = match icon_pixmap {
            ...
            Err(_,) => {
                item_proxy.icon_name().await.ok().as_deref().and_then(icon::icon_from_name,)
            }
        };
...
impl TrayService
{
    ...
    ) -> anyhow::Result<Layout,>
    {
        ...
        menu_proxy
            .event(id, "clicked", &value, chrono::offset::Local::now().timestamp_subsec_micros(),)
            .await?;
    ...
    fn update(&mut self, event: Self::UpdateEvent,)
    {
        match event {
            TrayEvent::Registered(new_item,) => {
                match self.data.0.iter_mut().find(|item| item.name == new_item.name,) {
                    Some(existing_item,) => {
