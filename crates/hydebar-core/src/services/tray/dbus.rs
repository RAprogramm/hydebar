...
impl StatusNotifierWatcher
{
    pub async fn start_server() -> anyhow::Result<Connection,>
    {
        ...
        connection.object_server().at(OBJECT_PATH, StatusNotifierWatcher::default(),).await?;
        let interface =
            connection.object_server().interface::<_, StatusNotifierWatcher>(OBJECT_PATH,).await?;
        ...
        tokio::spawn(async move {
            ...
            while let Some(evt,) = name_owner_changed_stream.next().await {
                ...
                if args.name.as_ref() == NAME {
                    ...
                    if let Some(idx,) =
                        interface.items.iter().position(|(unique_name, _,)| unique_name == name,)
                    {
                        let emitter =
...
impl StatusNotifierWatcher
{
    ...
    )
    {
        ...
        Self::status_notifier_item_registered(&emitter, &service,).await.unwrap();
