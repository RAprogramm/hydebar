...
impl NetworkBackend for NetworkDbus<'_,>
{
    ...
    {
        for device_path in self.wireless_access_points().await?.iter().map(|ap| ap.path.clone(),) {
...
impl<'a,> NetworkDbus<'a,>
{
    ...
    ) -> anyhow::Result<impl Stream<Item = anyhow::Result<NetworkEvent,>,> + 'a,>
    {
        ...
        for access_point in wireless_access_points {
            let ssid = access_point.ssid.clone();
            let proxy =
                AccessPointProxy::builder(conn,).path(access_point.path.clone(),)?.build().await?;
    ...
    pub async fn wifi_device_present(&self,) -> anyhow::Result<bool,>
    {
        ...
        for d in devices {
            let device =
                DeviceProxy::builder(self.0.inner().connection(),).path(d,)?.build().await?;
    ...
    ) -> anyhow::Result<Vec<KnownConnection,>,>
    {
        ...
        for c in known_connections {
            ...
            if wifi.is_some() {
                let ssid =
                    s.get("connection",).and_then(|c| c.get("id",),).map(|s| match s.deref() {
                ...
                let id =
                    s.get("connection",).and_then(|c| c.get("id",),).map(|v| match v.deref() {
                        Value::Str(v,) => v.to_string(),
                        _ => "".to_string(),
                    },);
    ...
    pub async fn wireless_access_points(&self,) -> anyhow::Result<Vec<AccessPoint,>,>
    {
        ...
        let wireless_access_point_futures: Vec<_,> = wireless_devices
            ...
            .map(|path| async move {
                ...
                let aps =
                    aps.into_values().sorted_by(|a, b| b.strength.cmp(&a.strength,),).collect();
...
mod tests
{
    ...
    {
        let states =
            vec![ConnectivityState::Portal, ConnectivityState::Loss, ConnectivityState::Full];
