...
impl MockHyprlandPort
{
    pub fn with_active_window(title: &str, class: &str,) -> Self
    {
        ...
        *port.active_window.lock().expect("poisoned active window lock",) =
...
impl HyprlandPort for MockHyprlandPort
{
    ...
    {
        Ok(self.active_window.lock().expect("poisoned active window lock",).clone(),)
    }

    fn workspace_snapshot(&self,) -> Result<HyprlandWorkspaceSnapshot, HyprlandError,>
    {
        Ok(self.workspace_snapshot.lock().expect("poisoned workspace snapshot lock",).clone(),)
    }
    ...
    {
        Ok(self.keyboard_state.lock().expect("poisoned keyboard state lock",).clone(),)
    }
