...
impl Outputs
{
    ...
    {
        self.0.iter().any(|(n, info, _,)| info.is_some() && n.as_deref() == Some(name,),)
    }
    ...
    ) -> Task<Message,>
    {
        ...
        if target {
            ...
            let destroy_task =
                match self.0.iter().position(|(key, _, _,)| key.as_deref() == Some(name,),) {
                    Some(index,) => {
    ...
    ///     Outputs::new::<(),>(config.appearance.style, config.position, &config,);
    /// let task =
    ///     outputs.sync::<()>(config.appearance.style, &config.outputs, config.position, &config,);
    /// # let _ = task;
    /// ```
    ...
    ) -> Task<Message,>
    {
        ...
        {
            debug!("Repositioning output: {:?}, new position {:?}", shell_info.id, position);
            shell_info.position = position;
...
mod tests
{
    ...
    fn toggle_menu_opens_and_closes()
    {
        ...
        let (mut outputs, _task,) =
            Outputs::new::<(),>(config.appearance.style, config.position, &config,);
        let id = outputs.iter_internal().next().unwrap().1.as_ref().unwrap().id;

        let button_ref = ButtonUIRef {
            position: Point::new(0.0, 0.0,), viewport: (0., 0.,),
        };
