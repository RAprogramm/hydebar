...
///
/// Instances manage Wayland layer-surfaces for both the main bar surface and the
/// associated menu surface per monitor. All operations return [`Task`] objects
/// that must be executed by the caller to coordinate with the compositor.
///
/// # Examples
///
/// ```
...
impl Outputs {
    ...
    /// compositor-side layer-surfaces. When the monitor name is not requested by
    /// configuration the [`Task`] is empty and the state records the Wayland
    /// output for future synchronisation.
    ///
    /// # Examples
    ///
    /// ```ignore
    ...
    /// required to add or remove surfaces as well as to update style or position
    /// changes.
    ///
    /// # Examples
    ///
    ...
    /// # use hydebar_core::config::Config;
    /// let config = Config::default();
    /// let (mut outputs, _task) = Outputs::new::<()>(
    ///     config.appearance.style,
    ///     config.position,
    ///     &config,
    /// );
