...
/// the UI queue and a [`Handle`] tied to the runtime powering background tasks.
/// Modules can use the handle to spawn asynchronous work; those tasks must
/// cooperate with cancellation by completing promptly when dropped. Tokio
...
impl ModuleContext {
    ...
    /// cancellation. When a task is aborted, Tokio guarantees that the future
    /// is dropped without panicking, ensuring that no partially published
    ...
    /// # Preconditions
    ///
    /// - `convert` must transform the module-specific payload into a
    ///   [`ModuleEvent`].
    ///
    /// # Examples
    ///
