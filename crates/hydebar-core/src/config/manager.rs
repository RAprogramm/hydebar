...
impl ConfigManager
{
    ...
    where
        F: FnOnce(&Config,) -> T,
    {
        self.state.read().map_err(|_| ConfigManagerError::Poisoned,).map(|guard| f(&guard,),)
    }
...
fn compute_impact(previous: &Config, next: &Config,) -> ConfigImpact
{
    ...
    mark_if_changed(&mut impact, ModuleName::Updates, &previous.updates, &next.updates,);
    mark_if_changed(&mut impact, ModuleName::Workspaces, &previous.workspaces, &next.workspaces,);
    mark_if_changed(
        &mut impact,
    ...
    mark_if_changed(&mut impact, ModuleName::SystemInfo, &previous.system, &next.system,);
    mark_if_changed(&mut impact, ModuleName::Battery, &previous.battery, &next.battery,);
    mark_if_changed(&mut impact, ModuleName::Clock, &previous.clock, &next.clock,);
    mark_if_changed(&mut impact, ModuleName::Settings, &previous.settings, &next.settings,);
    mark_if_changed(
        &mut impact,
...
)
{
    let previous_map: HashMap<&str, &CustomModuleDef,> =
        previous.iter().map(|module| (module.name.as_str(), module,),).collect();
    let next_map: HashMap<&str, &CustomModuleDef,> =
        next.iter().map(|module| (module.name.as_str(), module,),).collect();
    ...
    for (name, module,) in &next_map {
        ...
        if needs_update {
            impact.affected_modules.insert(ModuleName::Custom((*name).to_string(),),);
        }
    }
