    ...
    Renderer: iced::advanced::Renderer,
{
    ...
    ) -> layout::Node
    {
        ...
        let mut calculate_edge_layout =
            |i: usize, (child, tree,): (&Element<'a, Message, Theme, Renderer,>, &mut Tree,)| {
                ...
                let (max_width, max_height,) =
                    (remaining, if fill_cross_factor != 0 { cross } else { max_cross },);
        ...
        nodes[2]
            .move_to_mut(Point::new(limits.max().width + self.padding.right, self.padding.top,),);
        ...
        let size =
            limits
                .resolve(self.width, self.height, Size::new(intrinsic_width, intrinsic_height,),);
